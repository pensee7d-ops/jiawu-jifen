...
2.2 会扩展 `stages` 的额外时长兑换配置，并新增 `time_exchange_requests`。现有阶段
迁移后默认关闭人工兑换，避免未确认的积分规则立刻生效。升级和回滚继续沿用上面的
备份、拉取、安装依赖、重启及数据库恢复流程，不新增系统服务或开放端口。

启动后依次检查：管理者首页只有状态和待办提示；底部“管理”进入统一管理中心；
`/activity` 同时显示当前任务和按日期排列的打卡/任务事件；`/computer` 能按角色显示
开关机或管理页签。最后在当前阶段设置兑换比例并启用，用测试申请验证审核批准后
积分流水与时长流水同时产生。未处理的兑换申请会在逻辑日或所属阶段结束后自动过期。
